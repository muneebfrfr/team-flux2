...
export default function DeprecationSection({
  ...
  // Linking state
  const [selectedDeprecationId, setSelectedDeprecationId] = useState<string>("");
  const [availableTechnicalDebts, setAvailableTechnicalDebts] = useState<TechnicalDebt[]>([]);
    ...
    try {
      ...
      const enrichedDeprecations = await Promise.all(
        deprecationsData.map(async (dep) => {
          if (dep.linkedTechnicalDebtIds && dep.linkedTechnicalDebtIds.length > 0) {
            try {
              const techDebtRes = await axios.post("/api/technical-debt/batch-get", {
              ...
              console.error("Failed to fetch linked technical debts for", dep.id);
  ...
  const fetchAvailableTechnicalDebts = useCallback(async () => {
    try {
      ...
      setAvailableTechnicalDebts(Array.isArray(res.data.data) ? res.data.data : []);
    } catch {
  ...
  const handleLinkDialogOpen = (deprecationId: string) => {
    ...
    const available = availableTechnicalDebts.filter((td) => !currentLinkedIds.includes(td.id));
  ...
  const handleLinkSubmit = async () => {
    ...
    try {
      await axios.post(`/api/deprecations/${selectedDeprecationId}/link-technical-debt`, {
  ...
  const handleUnlink = async (deprecationId: string, techDebtId: string) => {
    if (confirm("Are you sure you want to unlink this technical debt?")) {
      try {
        await axios.delete(`/api/deprecations/${deprecationId}/link-technical-debt`, {
    ...
    <ThemeRegistry> 
      <Box mt={4}>
                ...
                <CardContent>
                  ...
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                    ...
                    <Box display="flex" gap={1} alignItems="center">
                      ...
                      />
                      {dep.linkedTechnicalDebts && dep.linkedTechnicalDebts.length > 0 && (
                        <Chip
                          icon={<DebtIcon />}
                          ...
                          <Box key={techDebt.id}>
                            <ListItem sx={{ py: 1 }}>
                                ...
                                secondary={
                                  <Box component="span" display="flex" gap={2}>
                                    ...
                                    <Chip
                                      ...
                                      variant="outlined"
                                    />
                                    <span>Owner: {techDebt.owner?.name || "—"}</span>
                                  </Box>
                                }
                              />
                              <ListItemSecondaryAction>
                                <IconButton
                                  size="small"
                                  onClick={() => handleUnlink(dep.id, techDebt.id)}
                                  color="error"
                            ...
                            {index < dep.linkedTechnicalDebts!.length - 1 && <Divider />}
                          </Box>
                        ))}
                      </List>
                    </Box>
        ...
        <Dialog open={formOpen} onClose={() => setFormOpen(false)} fullWidth maxWidth="sm">
          <DialogTitle>{isEditMode ? "Edit Deprecation" : "Add New Deprecation"}</DialogTitle>
          <DialogContent>
            <AppFieldText
          ...
          <DialogContent>
            ...
            <FormControl fullWidth margin="normal">
              ...
              <Select
                ...
                onChange={(e) => setSelectedTechDebtIds(e.target.value as string[])}
                input={<OutlinedInput label="Select Technical Debts to Link" />}
                renderValue={(selected) => (
                  <Box sx={{ display: "flex", flexWrap: "wrap", gap: 0.5 }}>
                    {(selected as string[]).map((value) => {
                      const techDebt = availableTechnicalDebts.find((td) => td.id === value);
                      return (
                        <Chip
                          key={value}
                    ...
                    <MenuItem key={techDebt.id} value={techDebt.id}>
                      <Box>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          Status: {techDebt.status} | Priority: {techDebt.priority} | Owner: {techDebt.owner?.name || "—"}
                        </Typography>
                      </Box>
                    </MenuItem>
                  ))
            ...
            {selectedTechDebtIds.length > 0 && (
              <Alert severity="success" sx={{ mt: 2 }}>
                {selectedTechDebtIds.length} technical debt item(s) selected for linking.
              </Alert>
            )}
          </DialogContent>
          <DialogActions>
